...
export class UserService {
  ...
  static async addProductToCart(user: User, productId: string) {
    if (user.cart) {
      const newUser = {
        ...user,
        cart: [...user.cart.map((product) => product._id), productId],
      };
      return await $api.patch<User>(`/users/${user._id}`, newUser);
    }

    const newUser = { ...user, cart: [productId] };
    return await $api.patch<User>(`/users/${user._id}`, newUser);
  }
}
